...
class parent_sql{
			...
			foreach($vars as $k=>$v){
				...
				if(preg_match('/^[@_A-z]+[0-9@_A-z]*$/', $vt[0])){
					...
					switch($t){
						...
						break;
						case's':
							$rsql=str_replace($vt[0],mysqli_real_escape_string($db->db_id,$v), $rsql);
						break;
						case 'l': //список через ,
							/*$v = str_replace(" ", "", $v);
							if(preg_match('/^(\d+,)*\d+$/', $v)) {
								...
								$r = array('q'=>$rsql,'e'=>"'<b>$v</b>' '{$vt[0]}' is not valid list ");
							}*/
							$v = ( is_array($v)? implode(',',$v): str_replace('|',',', $v) );
							$rsql=str_replace($vt[0],preg_replace('/[^0-9,]/','',$v),$rsql);
						break;
					}
	...
	public function __construct($sql = NULL, $checkvars = false,$db_id='')
	{
		if ($sql !== NULL)
		{
			if(is_array($checkvars)){
				...
				//print_r($sql);
				//print_r($checkvars);
				file_put_contents(droot.'logs'.ds.'sql.log',file_get_contents(droot.'logs'.ds.'sql.log')."\n".print_r($sql,true)."\n".print_r($checkvars,true));
	...
	public function query($sql,$db_id='')
	{
		...
		$this->_sql = $sql;
		if($db_id!='')
			$this->q_res = mysqli_query($db_id,$this->_sql);
		elseif(is_object($db))
			$this->q_res = mysqli_query($db->db_id,$this->_sql);
		else$this->q_res = mysqli_query($this->_sql);
		if ($this->q_res===false){
		die("Invalid query: [".$this->_sql."] " . mysqli_error());
	...
	public function row($q_res = '',$type=MYSQLI_ASSOC)//MYSQL_NUM MYSQL_BOTH
	{
		if ($q_res == '') $q_res = $this->q_res;
		return mysqli_fetch_array($q_res,$type);
	}
	public function rows($q_res = '',$index='',$type=MYSQL_ASSOC)//MYSQL_NUM MYSQL_BOTH
	{
		...
		while ($line = mysqli_fetch_array($q_res, $type))
			$arr[$line[$index]]=$line;
		else
		while ($line = mysqli_fetch_array($q_res, $type))
			$arr[]=$line;
	...
	public static function get($sql,$checkvars = false,$index='',$type=MYSQL_ASSOC)
	{
		...
		$arr=array();
		if($index!='' and is_string($index))
		while ($line = mysqli_fetch_array($r, $type))
			$arr[$line[$index]]=$line;
		else
		while ($line = mysqli_fetch_array($r, $type))
			$arr[]=$line;
		...
		if(count($data)>0 and $table!=''){
			...
			if(is_array($values)){
				...
				if(($u?!(in_array($key,$unset)):true)){
					$keys[$key]="`".($key)."`";
					$vals[$key]="'".(mysqli_real_escape_string($db->db_id,$val))."'";
				}
				$v[]="(".(implode(',',$vals)).")";
			}elseif(($u?!(in_array($key,$unset)):true)){
				$keys[$key]="`".($key)."`";
				$vals[]="'".(mysqli_real_escape_string($db->db_id,$values))."'";
			}
			if(!$b){
				$values="(".(implode(',',$vals)).")";
				$c=1;
			...
			$r=$q->q_res;
			if($r){
				$r=array('start'=>mysqli_insert_id($db->db_id),'count'=>$c);
	...
	public static function up($table='',$data=array(),$where=array(),$escapestr=true)
	{
		...
		if(count($data)>0 and $table!=''){
			...
			foreach($data as $key=>$value){
				$set[$key]="`".($key)."`='".($escapestr?mysqli_real_escape_string($db->db_id,$value):$value)."'";
			}
